...
/*			//items slider is visible - hide slider and show the intro panel
	$('.cd-intro-block').on('click', function(event) {
		...
		if( $introSlider.hasClass('projects-visible') && !$(event.target).is('a[data-action="show-projects"]') ) {
			$introSlider.removeClass('projects-visible');
			$introPanel.removeClass('projects-visible');
		}
	});
*/
	
	//close items slider with button (as well as intro panel)
	$('.cd-projects-wrapper>a').on('click',function(event){
		var $introPanel = $(this).parent();
		var $introSlider = $introPanel.prev();
		if( $introSlider.hasClass('projects-visible') && $(event.target).is('a[data-action="close-projects"]') ) {
			$introSlider.removeClass('projects-visible');
			$introPanel.removeClass('projects-visible');
		}
	});
